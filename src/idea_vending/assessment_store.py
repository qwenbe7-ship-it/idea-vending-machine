...
class AssessmentStore:
    """Keep a bounded, expiring, thread-safe copy of completed assessments."""
    ...
    def _evict_expired_and_over_capacity(self) -> None:
        """Evict while the caller holds ``self._lock``."""
        now = self._clock()
        expired = [
            runtime_id
    ...
    def save_completed(self, result: dict[str, Any]) -> str:
        ...
        trusted_result = deepcopy(result)
        with self._lock:
            self._evict_expired_and_over_capacity()
            existing = self._records.get(runtime_id)
            if existing is not None:
                if existing["result"] == trusted_result:
                    return runtime_id
                raise ValueError("runtime_id collision with a different completed result")
